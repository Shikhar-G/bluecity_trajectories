...
def add_past_img(past_img, idx, main_object):
    img = past_images[main_object.id][idx]
    past_main_obj = obj_poses[main_object.id][0][idx-1] # this doesn't account for the minus count thing
    shift_x = main_object.centerX - past_main_obj.centerX
    shift_y = main_object.centerY - past_main_obj.centerY
    shift_x = shift_x*math.cos(past_main_obj.rotation) - shift_y*math.sin(past_main_obj.rotation)
    shift_y = shift_x*math.sin(past_main_obj.rotation) + shift_y*math.cos(past_main_obj.rotation)
...
def add_circle(img, centerX, centerY, length, width, color):
    ...
    # Create a white circle image with the same dimensions as the input image
    # print((centerX), (centerY), (radius), max(length, width) / 2)
    cv2.circle(img, (int(centerX), int(centerY)), radius, color, thickness=-1)
    return img

def frame_to_image_wrt_object(data, idx, count): # egocentric view wrt to object
    ...
    if not os.path.exists("test/"+main_object.id):
        os.makedirs("test/" + main_object.id)

    img = images[count]
    # img = np.zeros((400, 400, 3), dtype=np.uint8)
    # for obj in data.objects:
    #     x = (obj.centerX - main_obj_x)
    #     y = (obj.centerY - main_obj_y)
    #     # print ("here", obj.centerX, obj.centerY)
    #     img = add_circle(img, x + 200, y + 200, obj.length, obj.width, (255, 255, 255))
    ...
    # Apply the rotation to the image
    img = cv2.warpAffine(img, M, (w, h))
    past_images.setdefault(main_object.id, []).append(img)

    img = past_images[main_object.id][0]
    for i in range(1, len(past_images[main_object.id])):
        img = add_past_img(img, i, main_object)
    ...
    img = cv2.addWeighted(img, 1, future_pose_img, 1, 0)
    cv2.imwrite("test/{}/{:03d}.png".format(main_object.id, count), img)
...
# Use the unpickled data however you need
# print(type(data[0].objects))
process_pickle()
make_images()
# for i in range(len(images)):
#     cv2.imwrite("test1/{}.png".format(i), images[i])
# print(len(images))
# for key in obj_poses:
    # print(key, len(obj_poses[key]))
make_gif()